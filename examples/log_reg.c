...
modelForward(STORCH_CTX ctx, GraphNode* x, GraphNode* w, GraphNode* b)
{
  ...
  GraphNode* s = G_Sum(ctx, G_MatMul(ctx, w, x), b);
...
trainLoop(Dataset d, Tensor* w_v, Tensor* b_v)
{
  ...
  for (size_t epoch = 0; epoch < 500; epoch++) {
    ...
    for (size_t i = 0; i < d.n_samples; i++) {
      GraphNode* g_x = G_Value(
        ctx, T_Wrap(ctx, d.n_features, 1, d.features + (d.n_features * i)));
      GraphNode* g_y =
        G_Value(ctx, T_Wrap(ctx, 1, d.n_labels, d.labels + (d.n_labels * i)));
    ...
    printf("\n");
    // printf("%f %f %f\n", value(loss)->data[0], grad(b)->data[0],
    // value(b)->data[0]);
...
main(void)
{
  ...
  Dataset d = { .n_features = n_features,
                .n_labels = n_classes,
                .n_samples = n_samples,
                .features = features,
                .labels = targets };
