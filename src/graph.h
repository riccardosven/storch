...
typedef enum {
  NONE, VALUE, SUM, PRODUCT, DIFFERENCE, DIVISION, EXPONENTIAL, N_OPS
} Op;
...
GraphNode *G_Malloc(GRAPH_CTX ctx) {
  if (ctx->cap == ctx->len) {
    ctx->cap = 2*ctx->cap + 1;
...
GraphNode *G_New(GRAPH_CTX ctx, Op op, size_t arity) {
...
GraphNode* G_Product(GRAPH_CTX ctx, GraphNode *x, GraphNode *y) {
...
GraphNode* G_Value(GRAPH_CTX ctx, Tensor x) {
...
GraphNode *G_Sum(GRAPH_CTX ctx, GraphNode *x, GraphNode *y) {
...
GraphNode *G_Diff(GRAPH_CTX ctx, GraphNode *x, GraphNode *y) {
...
GraphNode *G_Div(GRAPH_CTX ctx, GraphNode *x, GraphNode *y) {
...
GraphNode *G_Exp(GRAPH_CTX ctx, GraphNode *x) {
